...
def readfield(rrfsfile,rrfsfile1,varname):
...
#   data=np.empty(arrayshape)
    ...
    #data=np.abs(ref3d[22,:,:])
    data=np.amax(ref3d,axis=0)
    data=np.abs(data)
    print(data.shape)

    for i in range(0,65):
      location=maximum_position(ref3d[i,:,:])
      print(i,np.max(ref3d[i,:,:]),location)
...
#     print('%s ref %s, sphum %s, T, %s '%(i,np.max(ref2d),np.max(sp2d), np.max(t2d)))
    ...
    im = plt.imshow(data, cmap='bwr', origin='lower', aspect='auto')
    ...
    # The label is set using the 'units' attribute read from the file
...
if __name__ == "__main__":
    ...
    print(rrfsfile)
    readfield(rrfsfile,rrfsfile1,varname)
    exit()
