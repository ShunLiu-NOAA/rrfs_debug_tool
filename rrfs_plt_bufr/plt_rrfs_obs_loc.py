...
#              'darkred','fuchsia','darkorchid','black'])
    ...
    cmap = colors.ListedColormap(['darkorange','red'])
...
#   cs = ax.scatter(x, y, tmp2m_1,s=10, marker='o',cmap=cmap,norm=norm)
    cs = ax.scatter(x1, y1, s=1, c='r', marker='.')
    cs = ax.scatter(x2, y2, s=1, c='b', marker='.')
#   cs = ax.pcolormesh(x, y, tmp2m_1,cmap=cmap,norm=norm)
#   cb = m.colorbar(cs, location='bottom',pad=0.05,extend='both')

    #plttitle="satwndbufr_"+vartype+"_"+cychr
    plttitle="satwhrbufr_amv_nesdis"
    plt.title(plttitle)
...
#   print(dat.shape)
    ...
    with open(diagfile,'r') as f:
      satid,lat,lon=np.loadtxt(f,usecols=(1,2,3),unpack=True)
    f.close()
...
#       lon2.append(lon[i])
    ...
    #print(lat[i],lon[i],satid[i])

    return lat, lon, satid


if __name__ == "__main__":

    stream = open("config_rrfs_obs_loc.yaml", 'r')
    ...
    cychr="goes_16_18,type 270 and 272"
    latconus, lonconus, satidconus = readobs(diagconus)
    dataconus=satidconus
